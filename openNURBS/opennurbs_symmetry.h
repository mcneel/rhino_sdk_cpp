...
public:
  ...
  */
  ...
  /// <summary>
  /// Find refection symmetries in NURBS curve control points.
  /// </summary>
  /// <param name="nurbs_curve"></param>
  /// <param name="distance_tolerance">
  /// This tolerance is used to decide when the reflection of
  /// one control point is close enought to another to
  /// qualify as a symmetric pair of control points.
  /// If distance_tolerance &gt; 0, then that value is used.
  /// Otherwise 0.0001 * nurbs_curve.BoundingBox().Diagonal().Length()
  /// is used.
  /// </param>
  /// <param name="angle_tolerance_radians">
  /// This tolerance is used to decide when a reflection plane's normal
  /// is close enough to a world axis that the world axis should be used.
  /// If 0 &lt; angle_tolerance_radians &lt;= pi/4, then that value is
  /// used. Otherwise  pi/360 (1/2 degree) is used.
  /// </param>
  /// <param name="reflecton_symmetries">
  /// The reflection planes are returned here.
  /// </param>
  /// <returns>
  /// Number of reflection planes that were found.
  /// </returns>
  static int FindNurbsCurveReflectionSymmetries(
    const ON_NurbsCurve& nurbs_curve,
    double distance_tolerance,
    double angle_tolerance_radians,
    ON_SimpleArray<ON_PlaneEquation>& reflecton_symmetries
  );

  /// <summary>
  /// If a NURBS curve is close to have a reflection symmetry, adjust
  /// its control points so it is exactly symmetric.
  /// </summary>
  /// <param name="reflection_plane_equation">
  /// The equation of the reflection plane.
  /// </param>
  /// <param name="test_tolerance">
  /// If the NURBS curve control points can be sorted into contiguous pairs
  /// such that the relfections or the pairs are with tolerance
  /// of each other, then the control point with the largest index
  /// is moved so the nurbs curve is exactly symmetric.
  /// If test_tolerance & gt; 0, then that value is used. Otherwise
  /// 0.05 * nurbs_curve.BoundingBox().Diagonal().Length() is used.
  /// </param>
  /// <param name="nurbs_curve"></param>
  /// <returns>
  /// True if the nurbs_curve was within tolerance of being symmetric
  /// and was modifed to be exatly symmetric.
  /// </returns>
  static bool MakeNurbsCurveReflectionSymmetric(
    ON_PlaneEquation reflection_plane_equation,
  ...
  /// <summary>
  /// If the NURBS curve's control points are symmetric within
  /// test tolerance, then they are made exactly symmetric.
  /// </summary>
