...
/*
Description:
  Create a .3dm path where the user has temp file access.
Parameters:
  prefix - [in] filename will start with the prefix.
  output - [out] full path of a filename.
Remarks:
  Function will return false if it fails for any reason. If it
  returns true the ouput is just a name, the file does not exist.
*/
ON_DECL
bool ON_GetTempFile(const wchar_t* prefix, ON_wString& output);
