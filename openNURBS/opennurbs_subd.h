...
#pragma region RH_C_SHARED_ENUM [ON_SubD::SubDFriendlyKnotType] [Rhino.Geometry.SubDFriendlyKnotType] [byte]
  ...
  */
  ...
  bBothSides - [in]
    If true, the input subd is offset both directions.
  bMakeSolid - [in]
    If true, the result is a solid made by adding quads connecting corresponding boundary edges
    the input subd and the offset or both sides of the offset. These edges are creases.
  ...
  */
  ...
  bBothSides - [in]
    If true, the input subd is offset both directions.
  solid_cap_edge_sharpness - [in]
    If ON_SubDEdgeSharpness::IsValidValue(solid_cap_edge_sharpness, true) is true, 
    the result is a solid made by adding quads connecting corresponding boundary 
    edges the input subd and the offset or both sides of a double offset.
    If you want a cap with smooth edges, pass ON_SubDEdgeSharpness::SmoothValue.
    If you want a cap with crease edges, pass ON_SubDEdgeSharpness::CreaseValue.
    If you want a cap with sharp edges, pass a value from 0 to ON_SubDEdgeSharpness::MaximumValue.
    If you do not want a cap, pass ON_DBL_QNAN.
  bPreserveSymmetry - [in]
    If both bPreserveSymmetry and this->ContentIsSymmetric() are true, then the returned sub will
    attempt to preserve the symmetry.
  destination_subd - [in]
  ...
  */
  ON_SubD* Offset(
    ...
    bool bOffsetSurface,
    bool bBothSides,
    double solid_cap_edge_sharpness,
    bool bPreserveSymmetry,
    ON_SubD* destination_subd
  ) const;
  ...
  */
  ...
  bOffsetSurface - [in]
    ...
    Otherwise the offset is calculated by offsetting the control net.
    Using interpolation is slower and typiclly produces less pleasing appearance.
    Using the control net typically means the surface offset distance is not 
    exactly offset_distance.
  bBothSides - [in]
    If true, the input subd is offset both directions.
  bMakeSolid - [in]
    If true, the result is a solid made by adding quads connecting corresponding 
    boundary edges the input subd and the offset or both sides of the offset.
  bPreserveSymmetry - [in]
    If both bPreserveSymmetry and this->ContentIsSymmetric() are true, then the returned sub will
    attempt to preserve the symmetry.
  destination_subd - [in]
  ...
  */
  ...
  /// Offset a SubD.
  /// </summary>
  /// <param name="offset_direction">
  /// The length of the vector determines the offset distance.
  /// The direction of the vector determines the offset direction.
  /// </param>
  /// <param name="bOffsetSurface">
  /// If true, the offset is calculated by interpolating offet surface points.
  /// Otherwise the offset is calculated by offsetting the control net.
  /// Using interpolation is slower and sometimes produces less pleasing appearance, especially when
  /// there are lots of vertices.  
  /// Using the control net typically means the surface offset distance is not
  /// exactly offset_distance.
  /// </param>
  /// <param name="bBothSides">
  /// If true, the input subd is offset both directions.
  /// </param>
  /// <param name="solid_cap_edge_sharpness">
  /// If ON_SubDEdgeSharpness::IsValidValue(solid_cap_edge_sharpness, true) is true,
  /// the result is a solid made by adding quads connecting corresponding boundary
  /// edges the input subd and the offset or both sides of a double offset.
  /// If you want a cap with smooth edges, pass ON_SubDEdgeSharpness::SmoothValue.
  ...
  /// If this and destination_subd are the same, the subd is offset in place.
  /// If nullptr is passed, the returned ON_SubD is on the heap and the caller
  /// is responsible for managing it. If you want the offset SubD to be managed by an ON_SubDRef,
  /// then pass &subdref.NewSubD() as destination_subd and subdref will manage
  /// the ON_SubD instance.
  /// </param>
  /// <returns>
  /// If the offset is successful, the result is returned.
  /// If the destination_subd is not nullptr, the result in in destination_subd.
  /// Otherwise the result in alllocated on the heap using new ON_SubD() and the caller
  /// is responsible for managing the ON_SubD instance.
  /// </returns>
  ON_SubD* Offset(
    ON_3dVector offset_direction,
    bool bOffsetSurface,
...
public:
  ...
  */
  ...
  /// <summary>
  /// SubDs have a mesh fragment cache that is caculated when the SubD is added to the Rhino
  /// document. The function
  /// ON_SubDDisplayParameters::AbsoluteDisplayDensityFromSubD(ON_SubDDisplayParameters::DefaultDensity, *this)
  /// is used to caculate the display density of that cache. When the SUbD has a valid cache,
  /// this function and the AbsoluteDisplayDensityFromSubD() call above will return identical results.
  /// When SubD edge sharpness is being edited inside a calculation, there are times when the cache
  /// is out of date, but it is updated before the modified SubD is added to the Rhino document.
  /// </summary>
  /// <returns>
  /// If chached mesh fragments exist, then the display density used to caclulate the cache is returned.
  /// Otherwise, 0 is returned.
  /// </returns>
  unsigned int MeshFragmentDensity() const;
#endif
  ...
  */
  ...
  /*
  Description:
    Validates that symmetry sets counts are valid for that SubD vertex.
  Parameters:
    vertex - [in]
    subd_symmetric_motif_count - [in]: Number of expected motifs in the symmetry set.
    bSilentError - [in]: if true, ON_Error is not called on finding an incorrect value.
  */
  bool IsValidSymmetrySetCount(
    const ON_SubDVertex* vertex,
    const unsigned subd_symmetric_motif_count,
    bool bSilentError
  ) const;

  /*
  Description:
    Validates that symmetry sets counts are valid for that SubD edge.
  Parameters:
    edge - [in]
    subd_symmetric_motif_count - [in]: Number of expected motifs in the symmetry set.
    bSilentError - [in]: if true, ON_Error is not called on finding an incorrect value.
  */
  ...
  /*
  Description:
    Validates that symmetry sets counts are valid for that SubD face.
  Parameters:
    face - [in]
    subd_symmetric_motif_count - [in]: Number of expected motifs in the symmetry set.
    bSilentError - [in]: if true, ON_Error is not called on finding an incorrect value.
  */
  ...
  /*
  Description:
    Validates that symmetry sets counts are valid for that SubD level.
  Parameters:
    level - [in]: SubD level to validate
    bCheckVertices - [in]
    bCheckEdges - [in]
    bCheckFaces - [in]
    bSilentError - [in]: if true, ON_Error is not called on finding an incorrect value.
  */
  bool IsValidSymmetrySetCounts(
    const class ON_SubDLevel* level,
    const bool bCheckVertices,
    const bool bCheckEdges,
    const bool bCheckFaces,
    const bool bSilentError
  ) const;

  /*
  Description:
    Validates that symmetry sets counts are valid for all SubD levels.
  Parameters:
    level - [in]: SubD level to validate
    bCheckVertices - [in]
    bCheckEdges - [in]
...
#pragma endregion
  ...
  */
  ...
  Parameters:
    ...
    subd_edge_id - [in]
    prev_proxy_edge - [in]
      If not nullptr and not pointing to the start of the edges array,
      then the returned edge will be after than prev_proxy_edge.
      This is useful when multiple proxy edges correspond to the same subd edge.
  Returns:
  ...
  */
  ...
  Parameters:
    ...
    subd_edge - [in]
    proxy_edges_out - [out]
      Will hold the edges found.
  Returns:
    True if the expected number of valid edges were found. False otherwise.
  */
  bool GetProxyBrepEdges(const ON_Brep* proxy_brep, const ON_SubDEdge* subd_edge, ON_SimpleArray<const class ON_BrepEdge*>& proxy_edges_out) const;
  /*
  Parameters:
    proxy_brep - [in]
      Brep created on this->ProxyBrep()
    subd_face_id - [in]
    prev_proxy_face - [in]
      If not nullptr and not pointing to the start of the faces array,
      then the returned face will be after than prev_proxy_face.
      This is useful when multiple proxy faces correspond to the same subd face.
  Returns:
  ...
  */
  ...
  Parameters:
    ...
    subd_face - [in]
    proxy_faces_out - [out]
      Will hold the faces found.
  Returns:
    True if the expected number of valid faces were found. False otherwise.
  */
  bool GetProxyBrepFaces(const ON_Brep* proxy_brep, const ON_SubDFace* subd_face, ON_SimpleArray<const class ON_BrepFace*>& proxy_faces_out) const;
#endif
