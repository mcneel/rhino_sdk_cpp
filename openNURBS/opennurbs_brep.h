...
public:
  ...
  */
  ...
  Parameters:
    ...
    N      - [out] Surface normals at edge_t.  Perpendicular to V.
  Returns:
  ...
  */
  ...
  Description:
    For each trim at this edge, find the 3d unit vector at the edge parameter in the trim's surface tan plane,
    perpendicular to the edge, which points to the active side of the trim. Also returns the point on the edge.
  Parameters:
    edge_t - [in] parameter of this edge.
    P      - [out] The point on the edge
    V      - [out] 3d vectors pointing into the faces.  One for each trim, in order of m_ti.
    N      - [out] Surface normals at edge_t.  Perpendicular to V.
  Returns:
    True if vectors can be found. False if edge tangent or surface normals can't be found,
    or if input is bad.
  */
  bool Get3dVectorsIntoFaces(double edge_t, ON_3dPoint& P, ON_SimpleArray<ON_3dVector>& V, ON_SimpleArray<ON_3dVector>& N) const;

  /*
  Description:
    For each trim at this edge, find the 3d unit vector at the edge parameter in the trim's surface tan plane,
    perpendicular to the edge, which points to the active side of the trim. Also returns the point and tangent on the edge.
  Parameters:
    edge_t - [in] parameter of this edge.
    P      - [out] The point on the edge
    T      - [out] The edge tangent vector
    V      - [out] 3d vectors pointing into the faces.  One for each trim, in order of m_ti.
    N      - [out] Surface normals at edge_t.  Perpendicular to V.
  Returns:
    True if vectors can be found. False if edge tangent or surface normals can't be found,
    or if input is bad.
  */
  bool Get3dVectorsIntoFaces(double edge_t, ON_3dPoint& P, ON_3dVector& T, ON_SimpleArray<ON_3dVector>& V, ON_SimpleArray<ON_3dVector>& N) const;
#endif
  /*
  Description:
...
class ON_CLASS ON_BrepQuadFacePack {
 ...
 public:
  // 2025-04-02, Pierre, RH-86605:
  // C++20: User-declared constructors (including defaulted ones)
  // prevent aggregate initialization. Go back to compiler-declared default constructors.
