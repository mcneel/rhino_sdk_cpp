...
public:
  ...
  unsigned SymmetrySetCount(unsigned symmetry_motif_count, ON_SubDComponentPtr cptr, const bool bSilentError) const;
...
public:
  ...
  // Kappa(i,j) = (m_kappas + i*m_kappa_stride0 + j*m_kappa_stride1)
  // k1 = Kappa(i,j)[0]
  // k2 = Kappa(i,j)[1]
  size_t m_kappa_stride0 = 0;
  size_t m_kappa_stride1 = 0;
  double* m_kappas = nullptr;
...
private:
  ...
  /*
  Description:
    Evaluate the values of the 4 cubic uniform B-spline basis functions.
  Parameters:
    t - [in] 0.0 <= t <= 1.0
  ...
  Remarks:
    ...
    span is 0 <= t <= 1, then
    C(t) = (Pbasis[0]*C0 + Pbasis[1]*C1 + Pbasis[2]*C2 + Pbasis[3]*C3).
  */
  static void EvCubicSpanPointBasis(
    double t,
    double Pbasis[4]
    );
  ...
  /*
   Description:
     Evaluate the values of the 1st derivatives of
     the 4 cubic uniform B-spline basis functions.
   Parameters:
     t - [in] 0.0 <= t <= 1.0
       Evaluation parameter
     Dbasis - [out]
   Remarks:
     ...
     C'(t) = (Dbasis[0]*C0 + Dbasis[1]*C1 + Dbasis[2]*C2 + Dbasis[3]*C3).
   */
  static void EvCubicSpanDerivativeBasis(
    double t,
    double Dbasis[4]
  );
  ...
  /*
   ...
   Remarks:
     ...
     C'(t) = (Dbasis[0]*C0 + Dbasis[1]*C1 + Dbasis[2]*C2 + Dbasis[3]*C3).
   */
  static void EvCubicSpanPointAndDerivativeBasis(
    double t,
    double Pbasis[4],
    double Dbasis[4]
  );
  ...
  */
  ...
  /// <summary>
  /// Calcululate a subd fragment quad mesh points, normals, and/or curvatures
  /// from the bi-cubic uniform B-spline surface.
  /// If m_points or m_normals is not 
  /// </summary>
  /// <param name="count">
  /// A grid of count x count points is evaluated from m_srf_cv.
  /// </param>
  /// <param name="point_i0">
  /// Offset into the m_points, m_normals, or m_kappa array.
  /// </param>
  /// <param name="point_j0">
  /// Offset into the m_points, m_normals, or m_kappa array.
  /// </param>
  /// <returns>True on success, false on error</returns>
  bool EvaluateCubicBispanSurface(
    unsigned int count,
