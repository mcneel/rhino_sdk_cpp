...
public:
	...
	*/
	...
	enum class ParamSerialMethod : unsigned int // Return values for ParameterSerializationMethod()
	{
		...
		XmlStream, // Parameters are serialized by ReadParameterXMLStream() and WriteParameterXMLStream() (not recommended).  Now deprecated for reading.
	};
	...
	/** Instead of overriding GetSimulation you can override this method to directly
		...
		This method is now deprecated.  Use GetSimulation.
		*/
	RDK_DEPRECATED virtual bool ReadSimulationXMLStream(wchar_t* wszStream);
	...
	enum class EmbedFilesChoice : unsigned int
	{
		kNeverEmbed  = 0, // Never embed support files.
		kAlwaysEmbed = 1, // Always embed support files.
		kAskUser     = 2, // Ask the user by displaying a message box.
		kRepeatLast  = 3, // Repeat according to the user's last message box reply.
	};
