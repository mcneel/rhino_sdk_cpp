...
public:
  ...
  // Description:
  //  Bind this conduit to a viewport in a way that no other conduit gets
  //  executed and possibly even the document is not drawn. This is useful for
  //  "preview" conduits use to draw one item without other geometry showing
  //  up (block preview).
  void BindExclusive(const ON_Viewport& viewport, bool drawDocument);
  bool IsExclusivelyBound(bool* drawDocument) const;
