...
public:
  ...
  */
  ...
  /*
  Description:
    Returns the current value of m_bPermitOsnap
  */
  ...
  */
  ...
  BOOL32 Constrain(const ON_SubD& subd, bool bAllowPickingPointOffObject = false);
  ...
  */
  ...
  /*
  Description:
    Returns true and optionally the string if m_bAcceptString is true and user is typing a string, but
    hasn't pressed Enter.
    Necessary for making OnMouseMove override pay attention to what is being typed at the command
    prompt in real time, and also after call to GetPoint() if you want the typed string to affect the result
    even if the user picked a point.
  */
  bool StringPreview(ON_wString* pString = nullptr) const;

  /*
  Description:
    Sets the string preview. Pass nullptr or empty string to clear the preview.
  */
  void SetStringPreview(const wchar_t* pString);
