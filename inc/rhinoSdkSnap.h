...
public:
  ...
  */
  ...
  Parameters:
    ...
    bSentToBack && bCheckVisibility - If both are true the snap location is considered occluded even if it's on the near side
    of opaque shaded objects.
  Returns:
     TRUE if point is in snap region
  */
  bool SnapToPoint(const ON_3dPoint& point, bool bCheckVisibility = false) const;
  bool SnapToPoint(const ON_3dPoint& point, bool bCheckVisibility, bool bSentToBack) const;
  ...
  */
  ...
  bool SnapToPointNoZ(const ON_3dPoint& point, bool bCheckVisibility, bool bSentToBack) const;
