...
public:
  ...
  */
  ...
  Remarks:
    Use DocProperties().PageUnitsAndTolerances() for page space.
  */
  //ON_DEPRECATED_MSG("Since v9.0. Please use ModelUnits() instead")
  ...
  Returns:
    Model space unit system.
  Remarks:
    Use PageUnits() for page space.
  */
  const ON_UnitSystem& ModelUnits() const;

  /*
  Returns:
    Page space unit system.
  Remarks:
    Use ModelUnits() for model space.
  */
  const ON_UnitSystem& PageUnits() const;

  /*
  Returns:
    Active space unit system.
  */
...
public:
...
#if defined(OPENNURBS_TAG_WIP)
  int LookupObject(const CRhinoTag&, ON_SimpleArray<CRhinoObject*>&) const;
  ...
  */
  ...
  //   CRhinoMeshObject::CRhinoMeshObject()
  //   CRhinoDoc::AddObject
  const CRhinoObject* AddGeometryObject(
...
public:
  ...
  */
  ...
  /*
  Description:
    Wrapper to run script on iOS within a NSTimer
   */
...
public:
  bool AddUserInterfaceObject(class CRhinoUserInterfaceObject* uiObject, const ON_UUID& userInterfaceGroupId);
  bool AddUserInterfaceObject(std::shared_ptr<class CRhinoUserInterfaceObject> uiObject, const ON_UUID& userInterfaceGroupId);
  /*
  Description:
    Removes all CRhinoUserInterfaceObjects from this document with a matching userInterfaceGroupId
  Returns:
    Number of objects removed
  */
  int RemoveUserInterfaceObjects(const ON_UUID& userInterfaceGroupId);
  bool RemoveUserInterfaceObject(unsigned int serialNumber);
  bool UserInterfaceObjectsWithGroupIdExist(const ON_UUID& userInterfaceGroupId) const;
  ON_ClassArray<std::shared_ptr<class CRhinoUserInterfaceObject>> FindUserInterfaceObjectsByGroupId(const ON_UUID& userInterfaceGroupId);
...
private:
  CRhinoObject* UNUSED__m_active_model_geometry;   // geometry from active model (can be on locked or hidden layers)
  CRhinoObject* UNUSED__m_active_model_idef_geometry; // instance definitions geometry
  CRhinoObject* UNUSED__m_deleted_geometry; // always belongs to active model

  // worksession reference model(s) geometry - never saved
  CRhinoObject* UNUSED__m_reference_model_geometry;   // worksession reference model(s) geometry
  CRhinoObject* UNUSED__m_reference_model_idef_geometry; // worksession reference model(s) instance definition geometry

  CRhinoDocTableReference m_model_geometry_tableref;

  // view camera icon
  class CRhCameraIcon* m_camera_icon = nullptr;
...
private:
  ...
  bool IsObjectReferenced
  (
    const CRhinoObject* obj,
    const ON_UuidList& model_viewports,
    const ON_UuidList& page_viewports,
    const ON_UuidList& referenced_idef_geometry
  ) const;
...
public:
  ...
  void EmergencyZero(); //Internal use only
...
public:
  ...
  CRhinoObjectIterator(const CRhinoObjectIterator&);
  const CRhinoObjectIterator& operator=(const CRhinoObjectIterator&) = delete;
...
private:
  ...
  bool SkipIt(const CRhinoObject&, bool&) const;
...
private:
  ...
  mutable class CRhinoObjectIterator_Private* m_iterator_private = nullptr;
};
...
/*
Description:
  The Rhino document table object types that can be purged.
  Values can be or-ed ( | ) together to specify more than one
  table object type.
*/
enum class RhinoDocPurgeFilter : unsigned int
{
  None             = 0,
  AnnotationStyles = 0x1U,
  Bitmaps          = 0x2U,
  BlockDefinitions = 0x4U,
  Environments     = 0x8U,
  Groups           = 0x10U,
  HatchPatterns    = 0x20U,
  Layers           = 0x40U,
