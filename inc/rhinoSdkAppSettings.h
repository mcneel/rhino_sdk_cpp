...
public:
  ...
  */
  ...
  /*
  Description:
    Get the "factory default" list of MRU file names.
  Arguments:
    None
  Returns:
    List of default MRU file names.
  */
  ON_ClassArray<ON_wString> GetDefaultMruFileNames(void) const;
...
private:
  ...
  double   m_curvature_tolerance_percent = 0.05;
...
public:
  ...
  int  m_max_gl_level = 46;     // used to cap the OpenGL operating level
  ...
  bool DefaultDisplayOpenGL() const;
  void SetOpenGLAsDefaultDisplay(bool enable);
private:
...
public:
  ...
  bool PerformBlackWhiteSwitching() const;
  void EnableBlackWhiteSwitching(bool on);
...
public:
  ...
  // Copy objects to the clipboard in both the current and previous Rhino clipboard formats.  This
  ...
  static bool HatchPatternsFolder(ON_wString& hatch_pattern_folder);
...
public:
  ...
  bool RotateViewAroundSelectedObjects() const;
  void SetRotateViewAroundSelectedObjects(bool);
...
public:
  ...
  enum {
    ...
    command_options_in_command_history,
  };
...
#pragma region class CRhinoSoftTransformSettings

class RHINO_SDK_CLASS CRhinoSoftTransformSettings
{
public:
  CRhinoSoftTransformSettings();
  ~CRhinoSoftTransformSettings();

  CRhinoSoftTransformSettings(const CRhinoSoftTransformSettings&);
  CRhinoSoftTransformSettings& operator=(const CRhinoSoftTransformSettings&);

  CRhinoSoftTransformSettings(CRhinoSoftTransformSettings&&);
  CRhinoSoftTransformSettings& operator=(CRhinoSoftTransformSettings&&);

  bool operator==(const CRhinoSoftTransformSettings&) const;
  bool operator!=(const CRhinoSoftTransformSettings&) const;

  void LoadProfile(CRhinoProfileContext&);
  void SaveProfile(CRhinoProfileContext&) const;
  void Defaults();

  static const CRhinoSoftTransformSettings Default;
...
public:
  ...
  bool MeasureDistanceAlong() const;
  void SetMeasureDistanceAlong(bool bAlong);

  ON_Color FalloffColor() const;
  void SetFalloffColor(ON_Color color);

  int CvCountU() const;
  void SetCvCountU(int iCvCountU);

  int CvCountV() const;
  void SetCvCountV(int iCvCountV);

  bool ShowSurfaceConstraintWidgets() const;
  void SetShowSurfaceConstraintWidgets(bool bShow);

private:

  bool m_bEnabled = false;
  double m_dRadius = 5.0;
  int m_iShape = 0;
  bool m_bMeasureDistanceAlong = false;
  ON_Color m_FalloffColor = ON_Color(0, 61, 255);
  int m_iCvCountU = 2;
  int m_iCvCountV = 2;
  bool m_bShowSurfaceConstraintWidgets = false;

private:
  ON__UINT_PTR m_private = 0;
};

#pragma endregion class CRhinoSoftTransformSettings
...
public:
  ...
  const CRhinoSoftTransformSettings& SoftTransformSettings() const;
  void SetSoftTransformSettings(const CRhinoSoftTransformSettings&);
  ...
  bool PointPicksHaveToBeOsnapped() const;
  void SetPointPicksHaveToBeOsnapped(bool bEnable);

  bool PickingIgnoresGrouping() const;
  void SetPickingIgnoresGrouping(bool bEnable);

  bool DemoteRealtimeViewsToRenderedOnLoad() const;
  void SetDemoteRealtimeViewsToRenderedOnLoad(bool enable);

  bool AutoHideGuides() const;
  void SetAutoHideGuides(bool bEnable);
