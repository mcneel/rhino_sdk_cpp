...
public:
  ...
  */
  ...
  enum result 
  {
    ...
    number_preview, // internal, user is typing a number - call NumberPreview() to get value
    point_preview, // internal, user is typing a 3d point - call Point() to get 3d point
    string_preview, // internal, user is typing a string - call StringPreview() to get the string
  ...
  */
  ...
  Parameters:
    ...
    number_value - [in/out] pointer to current value of the number.
        If the user changes the number's value, the value of 
  ...
  */
  ...
  //////////
  // If you want to allow the user to be able to type in a literal string
  // during CRhinoGetPoint::GetPoint(), CRhinoGetObject::GetObject(), etc.,
  // then call AcceptString() before calling GetPoint()/GetObject().
  // If the user chooses to type in a string, then the result code
  // CRhinoGet::string is returned and you can use CRhinoGet::String()
  // to get the value of the string.
  void AcceptLiteralString(bool b = true);

  //////////
  // If you want to allow the user to be able to press enter when done
  // during CRhinoGetPoint::GetPoint(), CRhinoGetObject::GetObject(), etc.,
  // then call AcceptEnterWhenDone() before calling GetPoint()/GetObject().
  // There are instances of CRhinoGet that prompt with "Press Enter when Done." yet do not call AcceptNothing().
  // With the graphical command UI, these instances need an additional call to AcceptEnterWhenDone() so the dialog
  ...
  */
  ...
  /*
  Description:
    Limit the amount of time GetPoint, GetObjects, GetString, etc., will wait
    for input.
  Parameters:
  ...
  */
  ...
  unsigned int WaitDuration() const;
  /*
  Description:
    If this instance is actively waiting on input, calling TimeoutNow will
    immediately exit the waiting loop and return Result::timeout
  */
  void SetImmediateTimeout();
...
public:
  ...
  bool AcceptLiteralStringValue() const;

  void GetPrompt(ON_wString& prompt,
