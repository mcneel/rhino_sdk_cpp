...
public:
  ...
  virtual void AddText(const wchar_t* text, const ON_Plane& plane, double height,
                       const ON_Font* font, const ON_UUID& obj_id) = 0;

  virtual void* EVF(const wchar_t* wszFunc, void* pvData) = 0;
};

class RHINO_SDK_CLASS IRhWidgetDrawCacheEx2 : public IRhWidgetDrawCacheEx
{
public:
  virtual ~IRhWidgetDrawCacheEx2() { }

  // Add a texture to display with a planar widget.
  virtual void AddTexture(const ON_Plane& plane, ON_Interval x, ON_Interval y,
                          const ON_Texture& tex, const ON_UUID& obj_id, double transparency) = 0;

  // Add a texture to display with a spherical widget.
  virtual void AddTexture(const ON_Sphere& sphere, double horz_sta, double horz_end, double vert_sta, double vert_end,
                          const ON_Texture& tex, const ON_UUID& obj_id, double transparency) = 0;

  // Add a texture to display with a cylindrical widget.
  virtual void AddTexture(const ON_Cylinder& cylinder, double horz_sta, double horz_end,
                          const ON_Texture& tex, const ON_UUID& obj_id, double transparency) = 0;
};
...
public:
  ...
  unsigned int DocumentSerialNumber(void) const;
