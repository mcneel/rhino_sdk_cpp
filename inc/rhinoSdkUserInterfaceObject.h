...
private:
  ...
  // What the object that has keyboard focus is holding, for an input method editor that
  // asks about it rather than being told. Everything is counted in characters of the text
  // the user can see, which is the text with any composition already dropped into it, the
  // same string Draw lays out. compositionStart is -1 when nothing is being composed.
  // False when no object has focus, or the one that has it is not a text box.
  static bool GetCompositionStateHandler(unsigned int rhinoViewSerialNumber,
    int& textLength,
    int& selectionStart,
    int& selectionLength,
  ...
  // The text the user can see in the object that has keyboard focus: its text with any
  // composition already dropped into it, the same string Draw lays out and the same string
  // the counts above are measured in. An input method editor reads this back to work out
  // what it is converting against. False when no object has focus, or the one that has it
  // is not a text box.
  static bool GetCompositionTextHandler(unsigned int rhinoViewSerialNumber, ON_wString& displayText);

  // Select all, copy, cut and paste, by the letter the shortcut is bound to - 'A', 'C', 'X'
  // or 'V', in either case. This is for a platform that claims those chords before any key
  // handler sees them, the Mac being one: there the shortcut arrives as an action on the
  // view rather than as a key, and this is how it reaches the object that has keyboard
  // focus. continueExecution is left true when no object took it, so the shortcut goes on
  // to mean what it usually means at a viewport.
  static void OnEditingShortcutHandler(unsigned int rhinoViewSerialNumber,
    unsigned int shortcutKey,
    bool& continueExecution);
