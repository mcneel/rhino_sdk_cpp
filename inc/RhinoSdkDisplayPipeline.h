...
public:
  static DisplayTechnologies EngineTechnology(const CRhinoDisplayEngine* engine);
...
public:
  ...
  void DrawShadedInstance(const CRhinoInstanceDefinition*, const CDisplayPipelineMaterial* material, const ON_Xform* pConstXform, CRhinoCacheHandle* cache);
  ...
  */
  ...
  // The following analysis visualization are patterened after the Zebra definitions above...
  void DrawEmapPreview(
    const ON_Brep* brep,
    const CRhinoEmapAnalysisSettings& eas,
    const ON_MeshParameters& mp,
    const ON_Color& color,
    CRhinoCacheHandle* cache
  );
  ...
  void DrawCurvaturePreview(
    const ON_Brep* brep,
    const CRhinoCurvatureAnalysisSettings& cas,
    const ON_MeshParameters& mp,
    const ON_Color& color,
    CRhinoCacheHandle* cache
  );
  ...
  void DrawDraftAnglePreview(
    const ON_Brep* brep,
    const CRhinoDraftAngleAnalysisSettings& das,
    const ON_MeshParameters& mp,
    const ON_Color& color,
    CRhinoCacheHandle* cache
  );
...
private:
  void DrawNestedObject(const CRhinoInstanceDefinition*, const ON_Xform* = nullptr, bool drawNow=false);
...
protected:
  ...
  bool  IsTransparentMaterial(const CDisplayAttributeMaterial&) const;
...
public:
  ...
  friend class CRhDisplayEnginePrivate_D3D11;
  friend class CRhDisplayEnginePrivate_D3D12;
};
