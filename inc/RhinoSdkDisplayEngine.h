...
public:
  ...
  void DrawString(const wchar_t* text, const ON_Color& textColor, const ON_2dPoint& point, double rotationRadians,
                  bool middle, float fontHeightPixels, const ON_Font* font);
...
public:
  ...
  bool ColorWritingEnabled() const;
...
public:
  ...
  enum class EDynamicRange : int
  {
    DR_STANDARD = 1,  // SDR quality using 8 bit precision
    DR_MEDIUM   = 2,  // HDR quality using 16 bit floating point precision
    DR_HIGH     = 3,  // HDR quality using 32 bit floating point precision
  };

  void SetDynamicRangeQuality(EDynamicRange dr);
  EDynamicRange DynamicRangeQuality(void) const;
...
public:
  ...
  const CRhinoDisplayPipeline*      DisplayPipeline() const;
