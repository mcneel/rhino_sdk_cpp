...
public:
  ...
  int SelectSubDComponents(
    const ON_SimpleArray<ON_SubDComponentPtr>& components,
    bool bSelect = true,
    bool bSynchHighlight = true,
    bool bPersistentSelect = false
  ) const;
...
public:
  ...
  */
  ...
  ON_DEPRECATED_MSG("OBSOLETE VERSION of MeshRef(): Call the overload with a bDisplayDensityIsAbsolute parameter.")
  ON_MeshRef MeshRef(
  ...
  /// <summary>
  /// Get an ordinary mesh of the SubD limit surface.
  /// </summary>
  /// <param name="mesh_type"></param>
  /// <param name="bDisplayDensityIsAbsolute"></param>
  /// <param name="display_density">
  /// When in doubt, pass 
  /// bDisplayDensityIsAbsolute = false and 
  /// display_density = ON_SubDDisplayParameters::DefaultDensity.
  /// ON_SubDDisplayParameters::MinimumDensity &lt=; display_density &lt;= ON_SubDDisplayParameters::MaximumDensity.
  ...
  */
  ...
  /*
  Description:
    Determines if the SubD has any hidden components (sub-objects).
  Returns:
    true  - if 1 or more components are hidden.
    false - if zero components are hidden.
  */
  bool HasHiddenComponents(void) const;
