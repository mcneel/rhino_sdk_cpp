...
public:
  ...
  */
  ...
  /*
  Description:
    Get the directory for the main rhino binary (RhinoCore). This is not
    necessarily the same directory as the top level application.
  */
  static ON_wString RhinoCoreDirectory();

  /*
  Description:
    Full path to the plug-ins directory for Rhino. This is where all core
    plugins are located in a Rhino installation. Ths is not the location for
    third party plugins
  */
  static ON_wString RhinoCorePluginsDirectory();

  // Description:
  //  Top level directory for the Rhino application
  static ON_wString RhinoInstallDirectory();
  ...
  */
  ...
  /*
    Description:
      Call this method to return a temporary path for mac or windows.
    Parameters:
      wResult [out]                - Reference to ON_wString which will receive the temporary path
    Returns:
      Returns true if a valid path can be found.
  */
  static bool GetTemporaryPath(ON_wString& wResult);
