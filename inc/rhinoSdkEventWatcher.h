...
/*
Description:
  The CRhinoUuidCollection class provides a tool to efficiently maintain a
  collection of uuids and determine if a uuid is in the collection.
  This class is useful when an event watcher needs to watch for events happening
...
public:
  ...
  */
  ...
  /*
   ...
   Parameters:
     doc - [in] The new current document or null if no documents are currently open.
  */
  ...
  */
  ...
  /*
  Description:
    Called if an object is deleted. At some later point the object can be undeleted.
  Parameters:
    doc - [in] object's document
  ...
  */
  ...
  /*
  Description:
    Called if an object is undeleted.
  Parameters:
    doc - [in] object's document
  ...
  */
  ...
  WARNING:
    ...
    any changes that are required in a CRhinoIsIdle.Notify() override.
  */
  virtual void OnDeselectObjects(
    CRhinoDoc& doc,
    const ON_SimpleArray<const CRhinoObject*>& objects
  );
  ...
  WARNING:
    ...
    any changes that are required in a CRhinoIsIdle.Notify() override.
  */
  virtual void OnDeselectAllObjects(
    CRhinoDoc& doc,
    int count
  );
  ...
  */
  ...
  WARNING:
    ...
    any changes that are required in a CRhinoIsIdle.Notify() override.
  */
  virtual void OnUpdateObjectMesh(
    CRhinoDoc& doc,
    CRhinoObject& object,
  ...
  Parameters:
    ...
    layer_index - [in] index of layer. If the event is layer_sorted, then layer_index is 0.
    old_settings - [in] If the event is layer_modified, then the old layer definition is
                   passed here. Otherwise, this parameter is nullptr.
  WARNING:
  ...
  */
  ...
  Parameters:
    ...
    linetype_index - [in] index of linetype. If the event is linetype_sorted, then linetype_index is 0.
  ...
  */
  ...
  Parameters:
    ...
    idef_index - [in] index of idef. If the event is idef_sorted, then idef_index is 0.
...
//
  ...
  */
  ...
  WARNING:
    ...
    any changes that are required in a CRhinoIsIdle.Notify() override.
  */
  virtual void TextureMappingTableEvent(
...
public:
  ...
  enum event_type
  {
    ...
    // Call when there is a change to the the analysis mode use by a Rhino object.
    event_type_rhino_object_analysis_mode_changed = 21,

    // Call when there is a change to the document's section style table.
    event_type_rhinodoc_section_style_table_changed = 22,

    // Call when there is a change to the document's markup table.
    event_type_rhinodoc_markup_table_changed = 23,
...
public:
  ...
  public:
    ...
    //   pointers and dereference them later.  Use
...
private:
  // prohibit use
  CRhinoOnViewEnableDrawingChanged(const CRhinoOnViewEnableDrawingChanged&) = delete;
  CRhinoOnViewEnableDrawingChanged& operator=(const CRhinoOnViewEnableDrawingChanged&) = delete;
};

class RHINO_SDK_CLASS CRhinoObjectAnalysisModeChanged : public CRhinoEventWatcherEx
{
public:
  CRhinoObjectAnalysisModeChanged(ON_UUID plugin_id);
  CRhinoObjectAnalysisModeChanged(ON_UUID plugin_id, bool headlessDocAware, bool headlessAppAware);
  ...
  public:
    ...
    // True if the analysis mode was enabled, or
    // false if the analysis mode was disabled.
  ...
  // Rhino will call Notify() immediately after view drawing enabled changes.
  // WARNING: 
  //   Never modify the Rhino document in a CRhinoObjectAnalysisModeChanged::Notify override.
  //   If you need to change the Rhino document or post update messages
  //   to controls, then your CRhinoObjectAnalysisModeChanged::Notify
  //   should record what happened in an efficient way and then make
  //   any changes that are required in a CRhinoIsIdle.Notify() override.
  virtual void Notify(const class CRhinoObjectAnalysisModeChanged::CParameters& params) = 0;

private:
  // prohibit use
  CRhinoObjectAnalysisModeChanged(const CRhinoObjectAnalysisModeChanged&) = delete;
  CRhinoObjectAnalysisModeChanged& operator=(const CRhinoObjectAnalysisModeChanged&) = delete;
};

// CRhinoDocSectionStyleTableChanged class
class RHINO_SDK_CLASS CRhinoDocSectionStyleTableChanged : public CRhinoEventWatcherEx
{
public:
  CRhinoDocSectionStyleTableChanged(ON_UUID plugin_id);
  CRhinoDocSectionStyleTableChanged(ON_UUID plugin_id, bool headlessDocAware, bool headlessAppAware);

  enum class EventType : unsigned char
  {
    SectionStyleAdded,
    SectionStyleDeleted,
    SectionStyleUndeleted,
    SectionStyleModified,
    SectonStyleSorted
  };
  ...
  public:
    ...
    // If the event is CRhinoDocSectionStyleTableChanged::EventType::SectionStyleModified, then
    // the old section section style. Otherwise, this parameter is nullptr.
    // WARNING:
    //   This objects may be deleted at any time after Notify is called.
    //   Do not save the pointer and dereference it later.
  ...
  // Rhino will call Notify() immediately after view drawing enabled changes.
  // WARNING: 
  //   Never modify the Rhino document in a CRhinoDocSectionStyleTableChanged::Notify override.
  //   If you need to change the Rhino document or post update messages
  //   to controls, then your CRhinoDocSectionStyleTableChanged::Notify
  //   should record what happened in an efficient way and then make
  //   any changes that are required in a CRhinoIsIdle.Notify() override.
  virtual void Notify(const class CRhinoDocSectionStyleTableChanged::CParameters& params) = 0;

private:
  // prohibit use
  CRhinoDocSectionStyleTableChanged(const CRhinoDocSectionStyleTableChanged&) = delete;
...
{
public:
  CRhinoDocMarkupTableChanged(ON_UUID plugin_id);
  CRhinoDocMarkupTableChanged(ON_UUID plugin_id, bool headlessDocAware, bool headlessAppAware);
  ...
  public:
    ...
    // If the event is CRhinoDocMarkupTableChanged::EventType::MarkupModified, then
    // the old markup. Otherwise, this parameter is nullptr.
    // WARNING:
    //   This objects may be deleted at any time after Notify is called.
    //   Do not save the pointer and dereference it later.
  ...
  // Rhino will call Notify() immediately after view drawing enabled changes.
  // WARNING:
  //   Never modify the Rhino document in a CRhinoDocMarkupTableChanged::Notify override.
  //   If you need to change the Rhino document or post update messages
  //   to controls, then your CRhinoDocMarkupTableChanged::Notify
  //   should record what happened in an efficient way and then make
  //   any changes that are required in a CRhinoIsIdle.Notify() override.
  virtual void Notify(const class CRhinoDocMarkupTableChanged::CParameters& params) = 0;
...
{
public:
  CRhinoDocPageViewGroupTableChanged(ON_UUID plugin_id);
  CRhinoDocPageViewGroupTableChanged(ON_UUID plugin_id, bool headlessDocAware, bool headlessAppAware);

  enum class EventType : unsigned char
  {
    PageViewGroupAdded,
    PageViewGroupDeleted,
    PageViewGroupUndeleted,
    PageViewGroupModified,
    PageViewGroupSorted
  };
  ...
  {
  public:
    // The serial number of the document which contains the pageview group table.
    unsigned int m_rhino_doc_sn;

    // The pageview group table event
    CRhinoDocPageViewGroupTableChanged::EventType m_event_type;

    // The index of the pageview group that was added, deleted, undeleted, or modified.
    int m_index;

    // If the event is CRhinoDocPageViewGroupTableChanged::EventType::PageViewGroupModified,
    // then the old section section style. Otherwise, this parameter is nullptr.
    // WARNING:
    //   This objects may be deleted at any time after Notify is called.
    //   Do not save the pointer and dereference it later.
    const ON_PageViewGroup* m_old_settings;
  ...
  // Rhino will call Notify() immediately after view drawing enabled changes.
  // WARNING: 
  //   Never modify the Rhino document in a CRhinoDocPageViewGroupTableChanged::Notify override.
  //   If you need to change the Rhino document or post update messages
  //   to controls, then your CRhinoDocPageViewGroupTableChanged::Notify
  //   should record what happened in an efficient way and then make
  //   any changes that are required in a CRhinoIsIdle.Notify() override.
  virtual void Notify(const class CRhinoDocPageViewGroupTableChanged::CParameters& params) = 0;
