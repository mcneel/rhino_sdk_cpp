...
CRhinoCommand::result RhinoMeshObjects(
  ...
  int& outputlayer // 0 == source, 1 == current
);
...
Parameters:
  mp - [in/out] in = current settings
  layer - [in/out] layer to put output on 0 = source, 1 = current
Returns:
  CRhinoCommand::result code
*/
RHINO_SDK_FUNCTION
CRhinoCommand::result RhinoScriptGetMeshParameters(ON_MeshParameters& mp, int* outputlayer);
...
/*
Description:
  Performs same procedure as the MatchMeshEdge command on a multiple meshes.

Parameters:
  input -        [in]  meshes to split using information in following parameters.
  output -       [out] resulting split up meshes
  distance -     [in]  Distance to not exceed when modifying the mesh.
  simpleSplits - [in]  When this is true and there is only one split in a face the code just divides the face at that vertex.  If
  ...
  bRatchet -     [in]  Divides the initial distance by 1000 and goes through the code 4 times, each time increasing the distance
                       by a factor of 10.
  average -      [in]  Set to true if you want an average to the vertex sets to be modified
                       rather than using the first
  join -         [in]  Set to true if you want the output joined into a single mesh
*/
RHINO_SDK_FUNCTION
int RhinoMatchMeshEdgeEx(
  const ON_SimpleArray<const ON_Mesh*>& input,
