...
public:
  ...
  enum GEOMETRY_TYPE_FILTER
  {
    ...
    loop_object          = ON::object_type::loop_object, // brep or hatch loop
  ...
  */
  ...
  /*
  Description:
    The getobject filter for brep and hatch components (CRhinoGetObject::loop_object) is shared. In commands that
    deal exclusively with hatches and hatch components, it is necessary to prohibit brep and brep
    component selections.
  Parameters:
    bEnable - [in]
      true - (default)
        Selection of breps and brep components is determined by the standard filters.
      false -
        Selection of breps and brep components is prohibited. This is useful in commands that
        only want to select hatch components.
  Remarks:
    Default = true;
  */
  void EnableBrepSelections(bool bEnable);
  ...
  Remarks:
    Default = true;
  */
  bool BrepSelections() const;

  /*
Description:
  The getobject filter for brep and hatch components (CRhinoGetObject::loop_object) is shared. In commands that
  deal exclusively with breps or brep components, it is necessary to prohibit hatch
  component selections.
Parameters:
  bEnable - [in]
    true - (default)
      Selection of hatch components is determined by the standard filters.
    false -
      Selection of hatches and hatch components is prohibited. This is useful in commands that
      only want to select brep components.
Remarks:
  Default = true;
*/
  void EnableHatchSelections(bool bEnable);
  ...
  */
  ...
  //////////
  // By default, locked objects cannot be selected.
  // If you do want to be able to select locked
  // objects, then call EnableLockedObjectSelect(true).
  void EnableLockedObjectSelect(bool = true);
  bool LockedObjectSelect() const;
...
private:
  ...
  bool m_bChangedSetFilterOrSubSelect;
  bool m_bEnableLockedObjectSelect;
  unsigned char m_flags0 = 0; // 0x01 = prohibit brep selections, 0x02 = prohibit hatch selections
